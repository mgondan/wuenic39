...
% Save all estimates in (country).pl.v39.txt
estimate :-
    ...
    wuenic(Code, Vaccine, Year, Rule, Explanation, Coverage,
        ...
        Estimates),
    concat_atom(['out/', Code, '.pl.v39.txt'], File),
    open_out_file(Out, File,
...
wuenic_I(C, dtp1, Y, Rule, Expl, Coverage) :-
    ...
    !,
    Rule = 'RMF:',
    concat_atom(['DTP1 coverage estimated based on DTP3 coverage of ',
        DTP3, '. '], Expl),
...
% Before earliest anchor (of type reported)
wuenic_II(C, V, Y, Rule, Expl, Coverage) :-
    reported_time_series(C, V, Y, Source, Cov0),
    succ_anchor(C, V, Y, _Anchor, AnchorRule, _),
    AnchorRule = 'R: AP',
    !,
    Rule = 'R:',
    member(Source-Expl,
...
% Before earliest anchor (not of type "reported"): calibrated
wuenic_II(C, V, Y, Rule, Expl, Coverage) :-
...
% After latest anchor of type "reported"
wuenic_II(C, V, Y, Rule, Expl, Coverage) :-
    reported_time_series(C, V, Y, Source, Cov0),
...
% After latest anchor (not of type "reported"): calibrate
wuenic_II(C, V, Y, Rule, Expl, Coverage) :-
    reported_time_series(C, V, Y, _, Cov0),
...
% Duplicated the rule to preserve order of explanations in V3 output file
survey_reason_to_exclude(C, V, Y, Expl) :-
    survey_for_analysis(C, V, Y, ID, Description, _),
...
reported_reason_to_exclude(C, V, Y, Expl) :-
    ...
    concat_atom(
      [ 'Reported data excluded due to an increase from ', CoveragePrec,
        ' percent to ', Coverage, ' percent with decrease ',
